...
class SleepPy:
    ...
    *** Support for data from other wrist worn accelerometers will be added in the future. To maximize generality an
    ...
    """
    def __init__(self, input_file, results_directory, sampling_frequency, start_buffer='0s', stop_buffer='0s',
        ...
        self.sub_dst = results_directory + '/' + self.src_name  # create output directory
        self.fs = sampling_frequency  # save sampling frequency
        self.window_size = 60  # define window size in seconds
        self.band_pass_cutoff = (0.25, 12.0)  # define the cutoffs for the band pass filter
    ...
    def split_days_geneactiv_csv(self):
        ...
        data = data.loc[data.index[0] + pd.Timedelta(self.start_buffer):data.index[-1] - pd.Timedelta(self.stop_buffer)]

        # cut to defined start and end times if specified
        if self.start_time and self.stop_time:
            self.start_time = pd.to_datetime(self.start_time, format="%Y-%m-%d %H:%M:%S:%f")
            self.stop_time = pd.to_datetime(self.stop_time, format="%Y-%m-%d %H:%M:%S:%f")
        ...
        for day in days:
            ...
            df = day[1].copy()
            available_hours = (len(df) / float(self.fs)) / 3600.
            if available_hours >= 6:
                count += 1
                dst = "/raw_days/{}_day_{}.h5".format(self.src_name, str(count).zfill(2))
    ...
    def extract_activity_index(self):
        ...
        # get days
        days = sorted([self.sub_dst + '/raw_days/' + i for i in os.listdir(self.sub_dst + '/raw_days/')])
        for day in days:
            count += 1
            ...
            while idx < len(df) - incrementer:
                ...
                temp = band_pass_filter(temp, self.fs, bp_cutoff=self.band_pass_cutoff, order=3)

                # activity index extraction
                bp_channels = [i for i in temp.columns.values[1:] if 'bp' in i]  # band pass filtered channels
                activity.append([start_time, activity_index(temp, channels=bp_channels).values[0][0]])
                idx += incrementer
            ...
            dst = "/activity_index_days/{}_activity_index_day_{}.h5".format(self.src_name, str(count).zfill(2))
            activity.to_hdf(self.sub_dst + dst, key='activity_index_data_24hr', mode='w')
    ...
    def wear_detection(self):
        ...
        for day in days:
            ...
            df_wear.to_hdf(self.sub_dst + '/wear_detection/wear_detection_day_{}.hdf'.format(str(count).zfill(2)),
                           key='wear_detection_24hr', mode='w')
            ...
            # save post rescoring
            df_wear.to_hdf(self.sub_dst + '/wear_detection/wear_detection_rescored_day_{}.hdf'.format(str(count).zfill(2)),
                           key='wear_detection_rescored_24hr', mode='w')
    ...
    def major_rest_period(self):
        ...
        for day in days:
            ...
            df = np.arctan(df['Z'] / ((df['X'] ** 2 + df['Y'] ** 2) ** 0.5)) * (180. / np.pi)  # get angle
            df = df.resample('5s').mean().fillna(0)  # get 5 second average

            # save intermediate data for plotting
            df.to_hdf(self.sub_dst + '/major_rest_period/5_second_average_arm_angle_day_{}.hdf'.format(str(count).zfill(2)),
                      key='arm_angle_data_24hr', mode='w')

            df = np.abs(df - df.shift(1))  # get absolute difference
            df = self.roll_med(df, 60)  # run rolling median 5 minute

            thresh = np.percentile(df.Data.dropna().values, 10) * 15.  # calculate threshold
            ...
            df.to_hdf(self.sub_dst + '/major_rest_period/rest_periods_day_{}.hdf'.format(str(count).zfill(2)),
                      key='rest_period_data_24hr', mode='w')
    ...
    def sleep_wake_predict(self):
        ...
        """
        try:
            os.mkdir(self.sub_dst + '/sleep_wake_predictions')  # set up output directory
        except OSError:
            pass
        count = 0

        # get days
        days = sorted([self.sub_dst + '/activity_index_days/' + i
        ...
        for day in days:
            ...
            df.to_hdf(self.sub_dst + '/sleep_wake_predictions/sleep_wake_day_{}.hdf'.format(str(count).zfill(2)),
                      key='sleep_wake_data_24hr', mode='w')
    ...
    def calculate_endpoints(self):
        ...
        # get days
        days = sorted([self.sub_dst + '/sleep_wake_predictions/' + i
                       for i in os.listdir(self.sub_dst + '/sleep_wake_predictions/')])

        # get major rest periods for each day
        mrps = pd.read_csv(self.sub_dst + "/major_rest_period/{}_major_rest_periods.csv".format(self.src_name),
                           parse_dates=True,
                           index_col='day')
        endpoints = []
        for day in days:
            ...
            pct_time_sleep = 100. * (len(df) - sum(df.values)) / float(len(df))
            ...
            sleep_onset_lat = (df.idxmin()[0] - df.index[0]).total_seconds() / 60.

            # number of wake bouts
            num_wake_bouts = 0
            wake_bout_df = df.copy()
            wake_bout_df['block'] = (wake_bout_df.sleep_predictions.diff().ne(0)).cumsum()
    ...
    def roll_med(self, df, num_samples):
        ...
        while idx < len(df) - num_samples:
            med.append([df.index[idx], df.iloc[idx:idx + num_samples].median()])  # get start index, std value
            idx += 1
        # format data frame
    ...
    def roll_std_60_minute(self, df):
        ...
        while idx < len(df) - int(900 * self.fs):  # run until we reach the end
            xyz = df.ix[idx:idx + int(3600 * self.fs)].std().values  # save std in x y and z
            rstd.append([df.index[idx], xyz[0], xyz[1], xyz[2]])  # get start index of window, std values
    ...
    def roll_max_range_60_minute(self, df):
        ...
        while idx < len(df) - int(900 * self.fs):  # run until we reach the end
            xyz = df.ix[idx:idx + int(3600 * self.fs)].max().values - \
                  df.ix[idx:idx + int(3600 * self.fs)].min().values  # save range in x y z
            rr.append([df.index[idx], xyz[0], xyz[1], xyz[2]])  # get start index of window, range values
            idx += int(900 * self.fs)  # increment indexer by 15 minutes
    ...
    def visualize_results(self):
        ...
        rdays = sorted([self.sub_dst + '/raw_days/' + i
                        for i in os.listdir(self.sub_dst + '/raw_days/')])
        # wear (no rescoring)
        wdays = sorted([self.sub_dst + '/wear_detection/' + i
                        for i in os.listdir(self.sub_dst + '/wear_detection/')
                        if 'rescored' not in i and '.hdf' in i])
        # wear (with rescoring)
        wdays_re = sorted([self.sub_dst + '/wear_detection/' + i
                           for i in os.listdir(self.sub_dst + '/wear_detection/')
                           if 'rescored' in i])
        # major rest (arm angle)
        mrdays_aa = sorted([self.sub_dst + '/major_rest_period/' + i
                            for i in os.listdir(self.sub_dst + '/major_rest_period/')
                            if 'angle' in i])
        # major rest (periods)
        mrdays_rp = sorted([self.sub_dst + '/major_rest_period/' + i
                            for i in os.listdir(self.sub_dst + '/major_rest_period/')
                            if 'angle' not in i and 'hdf' in i])
        # activity index (full 24 hours)
        aidays = sorted([self.sub_dst + '/activity_index_days/' + i
                         for i in os.listdir(self.sub_dst + '/activity_index_days/')])
        # sleep wake (full 24 hours)
        swdays = sorted([self.sub_dst + '/sleep_wake_predictions/' + i
                         for i in os.listdir(self.sub_dst + '/sleep_wake_predictions/')])
        # endpoints (graphs/charts per day)
        endpoints = pd.read_csv(self.sub_dst + '/sleep_endpoints/sleep_endpoints_summary.csv', index_col='day')
        ...
        for day in days:
            ...
            # get shared index
            idx = pd.date_range(start=raw.index[0].replace(hour=12,
                                                           minute=0,
                                                           second=0,
                                                           microsecond=0),
                                periods=1440,
            ...
            t_labels = ('Total Sleep Time(minutes)',
                        'Percent Time Asleep',
            ...
            plt.suptitle('Visual Report for Source: {}\nDay: {}\nDate: {}'.
                         format(self.src_name, day + 1, idx[0].date()), fontsize=25)
            hours = mdates.HourLocator(interval=1)
            h_fmt = mdates.DateFormatter('%H:%M')
            all_axes = (ax0, ax1, ax2, ax3)

            # plot table
            tbl = axt.table(cellText=t_vals,
                            colLabels=t_labels,
                            cellLoc='center',
                            rowLoc='center',
                            loc='center',
            ...
            # plot raw
            raw[['X', 'Y', 'Z']].plot(ax=ax0, lw=1).legend(bbox_to_anchor=(0, 1), fontsize=20)
            ax0.set_ylabel("")
            ax0.set_xlabel("")
            ...
            aindex.plot(ax=ax1,
                        lw=1,
                        color='#6fc276').legend(labels=['activity'],
            ...
            angle.plot(ax=ax2,
                       lw=1,
                       color='#b36ff6').legend(labels=['arm angle'],
                                               bbox_to_anchor=(0, 0.75),
                                               fontsize=20)
            ax2.set_ylabel('')
            ax2.set_xlabel("")
            ...
            df.plot(ax=ax3, lw=8, x_compat=True).legend(bbox_to_anchor=(0, 1.3),
                                                        fontsize=20)
            ax3.set_ylabel("")
            ax3.set_xlabel("")
            ...
            plt.savefig(self.sub_dst + '/reports/Visual_Results_Day_{}.pdf'.format(day + 1))
            plt.close()
    ...
    def rescore(self, df):
        ...
        for i in range(1, len(blocks) - 1):
            wear = blocks[i][1]['wear'].values[0]  # get whether or not the block is wear
            if wear:
                # get hour lengths of the previous, current, and next blocks
                prev, current, post = len(blocks[i - 1][1]) * 0.25, len(blocks[i][1]) * 0.25, len(blocks[i + 1][1]) * 0.25
                ...
                if current < 3 and current / (prev + post) < 0.8:
                    df['wear'][df.block == blocks[i][0]] = 0  # rescore the wear period as non wear
    ...
    def rescore_last_day(self, df):
        ...
        for i in range(1, len(blocks)):
            wear = blocks[i][1]['wear'].values[0]  # get whether or not the block is wear
            if wear and blocks[i][1].index[0] > last_day_index:  # if wear, and it's the last day
    ...
    def rescore(self, predictions):
        ...
        for t in range(len(rescored)):
            if rescored[t] == 1:
                ...
                if 14 < wake_bin:  # rule c: at least 15 minutes of wake, next 4 minutes of sleep get rescored
                    rescored[t:t + 4] = 1.0
                elif 9 < wake_bin < 15:  # rule b: at least 10 minutes of wake, next 3 minutes of sleep get rescored
                    rescored[t:t + 3] = 1.0
                elif 3 < wake_bin < 10:  # rule a: at least 4 minutes of wake, next 1 minute of sleep gets rescored
                    rescored[t] = 1.0
                wake_bin = 0
        ...
        for t in range(10, len(rescored) - 10):
            if rescored[t] == 0:
                ...
                if 0 < sleep_bin <= 6:
                    if sum(rescored[start_ind - 10:start_ind]) == 10.0 \
    ...
    with open(full_path, 'rb') as in_file:
        ...
        while full_line:
            ...
            if 'Page Time:' in line:
                time = pd.to_datetime(":".join(line.split(':')[1:])[0:-2], format='%Y-%m-%d %H:%M:%S:%f')
            ...
            if len(line) == 3600:
                ...
                bins = (struct.unpack('12s 12s 12s 10s 1s 1s', bin(int(hx, 16))[2:].zfill(48)) for hx in hexes)
                decode = pd.DataFrame(bins, columns=['X', 'Y', 'Z', 'Light', 'Button', '_'],
                                      index=pd.DatetimeIndex([time] * 300) + delta)

                # binary to decimal and calibration
                decode.X = decode.X.apply(lambda x: round((BitArray(bin=x).int*100.0 - x_offset) / x_gain, 4))
                decode.Y = decode.Y.apply(lambda x: round((BitArray(bin=x).int*100.0 - y_offset) / y_gain, 4))
                decode.Z = decode.Z.apply(lambda x: round((BitArray(bin=x).int*100.0 - z_offset) / z_gain, 4))
